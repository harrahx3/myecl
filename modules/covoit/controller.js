exports.getAllAdmin = function (req, res) {
	...
	var promises = [];
	req.database.query('SELECT * FROM covoit', function (error, result) {
		if (error) {
...
exports.add = function (req, res) {
	req.database.query("INSERT INTO covoit (title, description, appliants) VALUES (?, ?, ?);", [req.body.title, req.body.description, req.body.appliants], (error, result) => {
		if (error) {
			req.logger.error(error);
			res.sendStatus(500);
		} else {
...
exports.delete = function (req, res) {
	req.database.query("DELETE FROM covoit WHERE id = ?;", [req.body.id], function (error, result) {
		if (error) {
			req.logger.error(error);
			res.sendStatus(500);
		} else {
...
exports.update = function (req, res) {
	req.database.query("UPDATE covoit SET title = ?, description = ? WHERE id = ?;", [req.body.title, req.body.description, req.body.id], (error, result) => {
		if (error) {
			req.logger.error(error);
			res.sendStatus(500);
		} else {
...
exports.apply = function (req, res) {
	req.database.query('SELECT appliants FROM covoit WHERE id = ?;', [req.body.id], (error, result) => {
		if (error) {
			req.logger.error(error);
			res.sendStatus(500);
		} else {
			...
			if (!json.ids.includes(req.user.id)) {
				...
				req.database.query("UPDATE covoit SET appliants = ? WHERE id = ?;", [data, req.body.id], (errorApply, resultApply) => {
					if (errorApply) {
...
exports.quit = function (req, res) {
	req.database.query('SELECT appliants FROM covoit WHERE id=?;', [req.body.id], function (error, result) {
		if (error) {
			req.logger.error("Erreur lors de la requête");
			res.sendStatus(500);
		} else {
			...
			if (json.ids.includes(req.user.id)) {
				...
				var data = JSON.stringify(appliants);
				req.database.query("UPDATE covoit SET appliants = ? WHERE id = ?;", [data, req.body.id], (errorQuit, resultQuit) => {
					if (errorQuit) {
